...
int main(int argc,char * *argv)
{
    if(argc!=4)
    {
        fprintf(stdin," Usage : %s latency-time round-number package-number\n",argv[0]);
        exit(-1);
    }
    ...
    if(SimFes_config()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]config failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    if(!SimFes_ana_fifo)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init analog queue failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    SimFes_poi_fifo=SimFes_queue_init(sizeof(SGDPS_CALGCHG));
    if(!SimFes_poi_fifo)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init point queue failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init link failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }

    if(SimFes_init_station()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init station failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }

    if(SimFes_init_multi_upana()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init multi upanalog failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }

    if(SimFes_init_multi_uppoi()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init multi uppoint failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]sort multi upanalog by psid failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]sort multi uppoint by psid failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }

	if(SimFes_init_upana()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init upana failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }

    if(SimFes_init_uppoi()<0)
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]init uppoint failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    */
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]sort upanalog by psid failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]sort uppoint by psid failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
...
*/
    ...
    {
        fprintf(stdin,"%s:%d:%s	Init msg error!\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]create SimFes_ana_action thread failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]create SimFes_poi_action thread failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
#endif
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]create SimFes_ana_send thread failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
    ...
    {
        fprintf(stdin,"%s:%d:%s		[ERROR]create SimFes_poi_send thread failed\n",__FILE__,__LINE__,__FUNCTION__);
        exit(-1);
    }
#endif
    ...
    while(1)
    {
        ...
        {
            fprintf(stdin,"%s:%d:%s		[WARN]cannot join with thread SimFes_ana_tid:%s\n",__FILE__,__LINE__,__FUNCTION__,strerror(err));
        }
#ifdef SIMFES_POI_DEBUG
        if((err=pthread_join(SimFes_poi_tid,&tret))!=0)
        {
            fprintf(stdin,"%s:%d:%s		[WARN]cannot join with thread SimFes_poi_tid:%s\n",__FILE__,__LINE__,__FUNCTION__,strerror(err));
        }
#endif
        if((err=pthread_join(SimFes_ana_send_tid,&tret))!=0)
        {
            fprintf(stdin,"%s:%d:%s		[WARN]cannot join with thread SimFes_ana_send_tid:%s\n",__FILE__,__LINE__,__FUNCTION__,strerror(err));
        }
#ifdef SIMFES_POI_DEBUG
        if((err=pthread_join(SimFes_poi_send_tid,&tret))!=0)
        {
            fprintf(stdin,"%s:%d:%s		[WARN]cannot join with thread SimFes_poi_send_tid:%s\n",__FILE__,__LINE__,__FUNCTION__,strerror(err));
        }
#endif
    }
    return 0;
